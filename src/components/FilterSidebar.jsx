import { Box, TextField, Typography, FormControlLabel, FormGroup, Checkbox, Button } from '@mui/material';
...
export default function FilterSidebar({
  ...
  // Show only first 5 categories, with 'More' link
  const [showAllCategories, setShowAllCategories] = useState(false);
  const visibleCategories = showAllCategories ? categories : categories.slice(0, 5);

  // Double-sided price slider state
  const [sliderValue, setSliderValue] = useState([Number(priceMin) || 0, Number(priceMax) || 1000]);

  // Sync slider with min/max
  useState(() => {
    setSliderValue([Number(priceMin) || 0, Number(priceMax) || 1000]);
  }, [priceMin, priceMax]);
  ...
  return (
    <Box sx={{
      ...
      <TextField
        ...
        sx={{ mb: 2, fontSize: '0.85rem', background: mode === 'dark' ? '#333' : undefined, borderRadius: mode === 'dark' ? 1 : undefined, color: mode === 'dark' ? '#fff' : undefined }}
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2 }}>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <TextField
                ...
                sx={{ fontSize: '0.75rem', minWidth: 48, maxWidth: 60, width: 60, background: mode === 'dark' ? '#333' : undefined, color: mode === 'dark' ? '#fff' : undefined, borderRadius: mode === 'dark' ? 1 : undefined }}
      ...
      <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
        <Button variant="contained" color="primary" sx={{ fontSize: '0.85rem', py: 0.5 }}>Apply</Button>
        <Button variant="outlined" color="secondary" onClick={handleClearFilters} sx={{ fontSize: '0.85rem', py: 0.5, borderColor: mode === 'dark' ? '#90caf9' : undefined, color: mode === 'dark' ? '#90caf9' : undefined }}>Clear</Button>
      </Box>
    </Box>
  );
}
